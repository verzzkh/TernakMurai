        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <form method="GET" class="flex items-center space-x-2">
                ...
                <select name="jenis" class="px-3 py-2 border rounded-md text-sm">
                    ...
                    <option value="pro" {{ (isset($jenis) && $jenis === 'pro') ? 'selected' : '' }}>Pro</option>
                    <option value="free" {{ (isset($jenis) && $jenis === 'free') ? 'selected' : '' }}>Free</option>
                </select>
                            ...
                            <tr>
                                ...
                                <td class="px-3 py-2">
                                    @if($p->jenis_akun === 'pro')
                                        Unlimited
                                    @else
                                        {{ max(0, 100 - ($p->deteksi_terpakai ?? 0)) }} / 100
                                    @endif
                                </td>
                                <td class="px-3 py-2">
