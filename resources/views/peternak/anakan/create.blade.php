<x-layout>
            ...
            <div class="max-w-3xl mx-auto bg-white dark:bg-darker rounded-md shadow-md overflow-hidden">
                ...
                <div class="p-6 bg-gray-50 dark:bg-darker-2 border-b dark:border-primary-darker">
                    <div class="flex items-center justify-center space-x-8">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" name="sumber_anakan" id="sumber-peternakan" value="peternakan"
                                class="form-radio h-5 w-5 text-primary" checked>
                            <span class="ml-2 text-gray-700 dark:text-gray-300">Anakan Dari Kandang</span>
                        </label>
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="radio" name="sumber_anakan" id="sumber-luar" value="luar"
                ...
                <form action="{{ route('peternak.anakan.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div id="form-dari-kandang" class="p-6 space-y-6">
                        ...
                        <h2 class="text-xl font-medium border-b pb-3 dark:border-primary-darker">Pilih Indukan Asal</h2>
                        <!-- Pilih Indukan Jantan -->
                        <div class="mt-4"> <label for="indukan_jantan_id" class="text-gray-700 dark:text-gray-200">
                                Indukan Jantan <span class="text-red-500">*</span> </label> <select
                                id="indukan_jantan_id" name="indukan_jantan_id" required
                                ...
                                <option value="" disabled selected>Pilih Indukan Jantan</option>
                                @forelse($indukanJantan as $jantan)
                                    <option value="{{ $jantan->id }}"> {{ $jantan->nomor_ring }} —
                                    {{ $jantan->nama ?? 'Tanpa Nama' }} </option> @empty <option value=""
                                        disabled>Tidak ada indukan jantan terdaftar</option>
                                @endforelse
                            </select> @error('indukan_jantan_id')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div> <!-- Pilih Indukan Betina -->
                        <div class="mt-4"> <label for="indukan_betina_id" class="text-gray-700 dark:text-gray-200">
                                Indukan Betina <span class="text-red-500">*</span> </label> <select
                                id="indukan_betina_id" name="indukan_betina_id" required
                                class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-darker dark:text-gray-300 dark:border-gray-600 focus:border-primary dark:focus:border-primary focus:outline-none focus:ring focus:ring-primary focus:ring-opacity-40">
                                <option value="" disabled selected>Pilih Indukan Betina</option>
                                @forelse($indukanBetina as $betina)
                                    <option value="{{ $betina->id }}"> {{ $betina->nomor_ring }} —
                                    {{ $betina->nama ?? 'Tanpa Nama' }} </option> @empty <option value=""
                                        disabled>Tidak ada indukan betina terdaftar</option>
                                @endforelse
                            </select> @error('indukan_betina_id')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div> <!-- Info pasangan (opsional) -->
                        <div id="info-pasangan"
                            class="mt-4 hidden bg-gray-50 dark:bg-darker-2 p-4 rounded-md border border-gray-200 dark:border-gray-700">
                            <h3 class="font-medium text-gray-700 dark:text-gray-300 mb-2">Informasi Pasangan</h3>
                            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Indukan Jantan</p>
                                    <p class="text-gray-800 dark:text-gray-200" id="info-jantan">—</p>
                                </div>
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Indukan Betina</p>
                                    <p class="text-gray-800 dark:text-gray-200" id="info-betina">—</p>
                                </div>
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Perkawinan Terakhir</p>
                                    <p class="text-gray-800 dark:text-gray-200" id="info-perkawinan">—</p>
                                </div>
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Total Anakan Sebelumnya</p>
                                    <p class="text-gray-800 dark:text-gray-200" id="info-total-anakan">—</p>
                                </div>
                            </div>
                        </div>

                        <h2 class="text-xl font-medium border-b pb-3 mt-10 dark:border-primary-darker">Informasi Anakan
                        </h2>
               ...
               focus:ring-opacity-40"
                            ...
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                Pilih tanggal menetas anakan. Jika belum pasti, gunakan tanggal estimasi.
                            </p>
                        </div>
                            ...
                            <div class="mt-2 flex space-x-4">
                                <label class="inline-flex items-center">
                                    <input type="radio" name="jenis_kelamin" value="jantan"
                                        ...
                                        {{ old('jenis_kelamin', 'jantan') == 'jantan' ? 'checked' : '' }}>
                                ...
                                <label class="inline-flex items-center">
                                    <input type="radio" name="jenis_kelamin" value="tidak_diketahui"
                                        class="form-radio text-primary"
                                        {{ old('jenis_kelamin') == 'tidak_diketahui' ? 'checked' : '' }}>
                                    <span class="ml-2 text-gray-700 dark:text-gray-300">Belum Tahu</span>
                                </label>
                            </div>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Jenis kelamin dapat diubah nanti
                                jika belum yakin</p>
                            @error('jenis_kelamin')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                      ...
                      <!-- Foto Anakan -->
                        <div class="mt-4">
                            ...
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Foto dapat ditambahkan nanti
                                setelah jenis kelamin dapat diidentifikasi dengan jelas</p>
                        </div>
                        <div class="mt-6">
                    ...
                    <div id="form-dari-luar" class="hidden p-6 space-y-6">
                        <input type="hidden" name="sumber_anakan" value="eksternal">
                        <h2 class="text-xl font-medium border-b pb-3 dark:border-primary-darker">Informasi Anakan</h2>

                        <!-- Ring Number Preview -->
                        <div
                            class="bg-gray-50 dark:bg-darker-2 p-4 rounded-md border border-gray-200 dark:border-gray-700">
                            <p class="text-gray-700 dark:text-gray-300 text-md">
                                <span class="font-medium">Ring Number:</span>
                                <span id="ring-number-preview"
                                    class="text-primary dark:text-primary-light font-semibold"></span>
                        ...
                        <!-- Jenis Kelamin -->
                        <div class="mt-4">
                            <label for="jenis_kelamin_luar" class="text-gray-700 dark:text-gray-200">Jenis Kelamin
                                <span class="text-red-500">*</span></label>
                            <select id="jenis_kelamin_luar" name="jenis_kelamin_luar" required
                                class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-darker dark:text-gray-300 dark:border-gray-600 focus:border-primary dark:focus:border-primary focus:outline-none focus:ring focus:ring-primary focus:ring-opacity-40">
                                <option value="" disabled selected>Pilih Jenis Kelamin</option>
                                <option value="jantan" {{ old('jenis_kelamin_luar') == 'jantan' ? 'selected' : '' }}>
                                    Jantan</option>
                                <option value="betina" {{ old('jenis_kelamin_luar') == 'betina' ? 'selected' : '' }}>
                                    Betina</option>
                                <option value="tidak_diketahui"
                                    {{ old('jenis_kelamin_luar') == 'tidak_diketahui' ? 'selected' : '' }}>Belum Tahu
                                </option>
                            </select>
                        ...
                        <!-- Asal/Penjual -->
                        <div class="mt-4">
                            <label for="asal_penjual" class="text-gray-700 dark:text-gray-200">Asal/Penjual <span
                                    class="text-red-500">*</span></label>
                        ...
                        <div class="mt-4">
                            <h3 class="text-md font-medium text-gray-700 dark:text-gray-300">Informasi Indukan
                                (Opsional)</h3>
                            <div class="grid grid-cols-1 gap-4 mt-2 sm:grid-cols-2">
                                <div>
                                    <label for="indukan_jantan_info" class="text-gray-700 dark:text-gray-200">Indukan
                                        Jantan</label>
                                ...
                                <div>
                                    <label for="indukan_betina_info" class="text-gray-700 dark:text-gray-200">Indukan
                                        Betina</label>
                        ...
                        <h2 class="text-xl font-medium border-b pb-3 mt-10 dark:border-primary-darker">Status dan Harga
                        </h2>

                        <!-- Status Pertumbuhan -->
                        <div class="mt-4">
                            <label for="status" class="text-gray-700 dark:text-gray-200">Status Pertumbuhan <span
                                    class="text-red-500">*</span></label>
                            <select id="status" name="status" required
                                class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-darker dark:text-gray-300 dark:border-gray-600 focus:border-primary dark:focus:border-primary focus:outline-none focus:ring focus:ring-primary focus:ring-opacity-40">
                                <option value="" disabled selected>Pilih Status Pertumbuhan</option>
                                <option value="trotol" {{ old('status') == 'trotol' ? 'selected' : '' }}>Trotol
                                </option>
                                <option value="pastol" {{ old('status') == 'pastol' ? 'selected' : '' }}>Pastol
                                </option>
                                <option value="lomba" class="status-jantan"
                                    {{ old('status') == 'lomba' ? 'selected' : '' }}>Lomba</option>
                        ...
                        <div class="mt-4">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="harga_beli" name="harga_beli" required min="0"
                                    step="1000" value="{{ old('harga_beli') }}"
                                    class="block w-full px-10 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-darker dark:text-gray-300 dark:border-gray-600 focus:border-primary dark:focus:border-primary focus:outline-none focus:ring focus:ring-primary focus:ring-opacity-40">
                            </div>
                            @error('harga_beli')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    ...
                    <!-- Form actions -->
                    <div
                        class="flex items-center justify-end px-6 py-4 bg-gray-50 dark:bg-darker-2 border-t dark:border-primary-darker">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Helper to enable/disable all controls inside a container
            function setDisabled(container, disabled) {
                if (!container) return;
                const controls = container.querySelectorAll('input, select, textarea, button');
                controls.forEach(control => {
                    // Skip CSRF token so we don't accidentally disable it and break the request
                    if (control.name === '_token') return;
                    // Skip submit buttons so the main submit stays enabled/handled by outer logic
                    if (control.type === 'submit' || control.tagName === 'BUTTON') return;
            ...
            sourcePeternakan.addEventListener('change', function() {
                if (this.checked) {
                    ...
                    // enable kandang form, disable luar form
                    setDisabled(formDariKandang, false);
                    setDisabled(formDariLuar, true);
                }
            });
            ...
            sourceLuar.addEventListener('change', function() {
                if (this.checked) {
                    ...
                    // enable luar form, disable kandang form
                    setDisabled(formDariKandang, true);
                    setDisabled(formDariLuar, false);
                }
            });

            // Initialize disabled state on load
            // By default 'peternakan' is checked in the template, so disable luar controls
            if (sourcePeternakan.checked) {
                setDisabled(formDariKandang, false);
                setDisabled(formDariLuar, true);
            } else if (sourceLuar.checked) {
                setDisabled(formDariKandang, true);
                setDisabled(formDariLuar, false);
            }
                        ...
                        <div class="space-y-1 text-center">
                            <svg class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                                ...
                                <label for="foto_anakan_${i}" class="relative cursor-pointer bg-white rounded-md font-medium text-primary hover:text-primary-dark focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-primary">
                                    <span class="px-2 py-1 dark:bg-darker dark:text-gray-300">Pilih foto</span>
                                    <!-- Use array-style name so PHP/Laravel receives files as foto_anakan[] -->
                                    <input id="foto_anakan_${i}" name="foto_anakan[]" type="file" class="sr-only" accept="image/*">
                                </label>
                                <p class="pl-1 dark:text-gray-400">atau seret dan lepas</p>
                            </div>
        ...
        if (singleFoto && !singleFoto.hasAttribute('name')) {
            ...
            form.addEventListener('submit', function(event) {
                ...
                if (sourcePeternakan.checked) {
                    ...
                    // tanggal_pembelian removed from form and not required
                    ...
                    if (!fotoInputLuar.files || !fotoInputLuar.files.length) {
                        isValid = false;
                        fotoInputLuar.parentElement.parentElement.parentElement.parentElement.classList.add(
                            'border-red-500');
                    }
                }
            ...
            document.addEventListener('change', function(e) {
                if (e.target && e.target.id && e.target.id.startsWith('foto_anakan_') && e.target.type ===
                    'file') {
