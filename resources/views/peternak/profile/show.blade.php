    ...
    <main>
        <div class="px-4 py-6 max-w-4xl mx-auto">
            <div class="bg-white dark:bg-darker rounded-lg shadow p-6">
                ...
                <form action="{{ route('peternak.profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="password" name="password_confirmation" class="block w-full px-3 py-2 border rounded-md">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="nama_peternakan" value="{{ old('nama_peternakan', $peternak->nama_peternakan ?? '') }}" required
                                   class="block w-full px-3 py-2 border rounded-md">
                            @error('nama_peternakan') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="text-sm">Nomor Handphone</label>
                            <input type="text" name="nomor_handphone" value="{{ old('nomor_handphone', $peternak->nomor_handphone ?? '') }}"
                                   class="block w-full px-3 py-2 border rounded-md">
                            @error('nomor_handphone') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    ...
                    <div class="flex items-start space-x-4">
                        <div>
                            ...
                            @error('foto_profil') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            @if(!empty($peternak?->foto_profil))
                                <img src="{{ asset('storage/'.$peternak->foto_profil) }}" class="w-32 h-32 rounded-md object-cover shadow">
                            @else
                                <div class="w-32 h-32 rounded-md bg-gray-100 dark:bg-gray-800 flex items-center justify-center">No Image</div>
                            @endif
                        </div>
                    </div>
