...
<body class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-slate-900 dark:to-slate-800 flex items-center justify-center p-4">
    <div class="max-w-lg w-full bg-white dark:bg-darker rounded-2xl shadow-xl p-8 border border-gray-200 dark:border-primary-darker">
        ...
        <form method="POST" action="{{ route('register') }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                <label for="name" class="block text-sm font-medium text-text-primary mb-2">Nama Pengguna (login)</label>
            ...
            <div>
                <label for="nama_peternakan" class="block text-sm font-medium text-text-primary mb-2">Nama Peternakan</label>
                <input id="nama_peternakan" name="nama_peternakan" type="text" value="{{ old('nama_peternakan') }}" required
                    class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>

            <div>
                <label for="alamat" class="block text-sm font-medium text-text-primary mb-2">Alamat</label>
                <textarea id="alamat" name="alamat" rows="2" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('alamat') }}</textarea>
            </div>

            <div>
                <label for="nomor_handphone" class="block text-sm font-medium text-text-primary mb-2">Nomor Handphone</label>
